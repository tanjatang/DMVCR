...
loader_params = {'batch_size':16// NUM_GPUS, 'num_gpus':NUM_GPUS, 'num_workers':num_workers} #96###############################################
...
# optimizer_mem = Optimizer.from_params([x for x in model.named_parameters() if x[1].requires_grad and x[0] == 'memory_cell1' or x[0] == 'memory_cell2'] ,
...
#     # print(memory_cell)
#     # memory_cell_path = os.path.join(path,
#     #                           'memory_cell'+ '.npz')
...
#-----------------------------------------------------------------------------------------------------------
# model_path = os.path.join('/home/tangxueq/tmp', "memory_cell800.npz")
# model_state = model.detector.module.state_dict() if isinstance(model, DataParallel) else model.detector.state_dict()
# torch.save(model_state, model_path)
#-------------------------------------------------------------------------------------------------------------------------------

# memory_cell = model.memory_cell.data.cpu().numpy()
# print(memory_cell)
# np.savez(model_path, memory_cell=memory_cell)
# model_state = model.memory_cell.state_dict() if isinstance(model, DataParallel) else model.detector.state_dict()
# torch.save(model_state, model_path)
#--------------------------------------------------------------------------------------------------------------
model.eval()
val_probs = []
