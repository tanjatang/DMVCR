...
##################################################################################################################
        ...
        if training_mode == 0:
            ...
            qao_rep = torch.cat((attended_q,attended_o),dim = 2)
...
##################################################################
        ...
        output, (h_n, h_c) = self.lstm(qa_rep)
        output = output[:, -1, :]  # 取最后一层输出  [96*4,1024]
        output = output.view(-1,4,1024)
        # output_res = self.Dictionary(output, self.memory_cell) # [96*4,1024]
        # output_res = output_res.view(-1,4,1024)

        logits = self.final_mlp(output)#.squeeze(2       [94,4,1024]
        # print(logits)
