...
# ---------------------------------------------------------------------------------------------------------------------------
        ...
        qa_res = self.QAGA_transformer(
            a_rep.view(a_rep.size(0) * a_rep.size(1), a_rep.size(2), a_rep.size(3)),
            q_rep.view(q_rep.size(0) * q_rep.size(1), q_rep.size(2), q_rep.size(3)), None,
        ...
        aq_res = self.QAGA_transformer(
            q_rep.view(q_rep.size(0) * q_rep.size(1), q_rep.size(2), q_rep.size(3)),
            a_rep.view(a_rep.size(0) * a_rep.size(1), a_rep.size(2), a_rep.size(3)), None,
            None)  # [b x 4, q_len, 512]
        ...
        qai_res = self.GA_transformer(image_feature, qa_rep.view(qa_rep.size(0) * qa_rep.size(1), qa_rep.size(2), qa_rep.size(3)),None,None)
...
#------------------------------------Att reduce------------------------------------------------------------------------------------------
#----------------------------------------------------------------------------------------------------------------------------------------
        qa_rep_feature = self.qa_AttFlat(qa_res, None)
        aq_rep_feature = self.aq_AttFlat(aq_res, None)
        qai_rep_feature = self.qai_AttFlat(qai_res,None)

        feature = qa_rep_feature + aq_rep_feature + qai_rep_feature
