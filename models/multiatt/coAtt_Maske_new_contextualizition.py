...
#------------------------------------------------------------------------------------------------------------------------------
        ...
        qa = self.GA(q_rep.view(q_rep.shape[0] * q_rep.shape[1], q_rep.shape[2], q_rep.shape[3]),
            a_rep.view(a_rep.shape[0] * a_rep.shape[1], a_rep.shape[2], a_rep.shape[3]),None,answer_mask[...,None].view(answer_mask.size(0)*answer_mask.size(1),a_len,-1))
        # print(qa.shape,'//////////////',q_rep.shape,'||||||||||||',a_rep.shape,'wwwwwwwwwwwwwwwwwwwwww'
        ...
        qa_text = torch.cat((q_rep,a_rep),dim=2)

        qa_mask = torch.cat((question_mask,answer_mask),dim=-1)
        qa_len = qa_mask.size(-1)
        ...
        qao = self.GA(image_feature,qa_text.view(qa_text.shape[0] * qa_text.shape[1],qa_text.shape[2],qa_text.shape[3]),None,qa_mask[...,None].view(qa_mask.size(0)*qa_mask.size(1),qa_len,-1))#这里改了MHA的mask
...
#-----------------------------------------------------------------------------------------------------------------------------------------
        ...
        aq_rep = self.QAGA_transformer(qa,qao, None,
            None)  # [b x 4, q_len, 512]
...
class MHAtt(nn.Module):
    ...
    def att(self, value, key, query, mask):
        ...
        # print(query.shape,'qqqqqqqqqqqqqqqqqqqqqqqqqqqaaaaaaaaa',value.shape,'vvvvvvvvvvvvvvvvvvvvv',key.shape,'kkkkkkkkkkkkkkkkkkkkkkkkkkkkkk')
        ...
        #
        # # print(scores.shape,'fffffffffffffffffffffffffffffffffffffffffffffffffff')
        # att_map = F.softmax(scores, dim=-1)
        att_map = masked_softmax(scores, mask)
...
class SGA_transformer(nn.Module):
    def __init__(self):
        super(SGA_transformer, self).__init__()
...
class GA_transformer(nn.Module):
    def __init__(self):
        ...
        self.enc_list = nn.ModuleList([GA(512, 512) for _ in range(3)])
        # self.position = PositionalEncoding(512,0.1,5000)
...
class GA(nn.Module):
    ...
    def forward(self, x, y, x_mask, y_mask):
        ...
        x = self.norm2(x + self.dropout2(
            self.mhatt2(y, y, x, x_mask) #原本是y_mask
        ))
