...
#################################################################################################################################
        ...
        self.lstm_norm = torch.nn.BatchNorm1d(1000)
        ...
        self.linear_l = torch.nn.Linear(768,256)
      ...
      #####################################################################
        ...
        self.ED = MCA_ED(768,512)
        self.QAED = MCA_ED(768,768)
        self.END = MCA_END()
    ...
    def memory_cell_load(self, m1, m2):
        if os.path.isfile(m1) and os.path.isfile(m2):
            ...
            memory_init1 = torch.randn(self.dic_size, 128) / 100  # randn
            memory_init2 = torch.randn(self.dic_size, 128) / 100
...
##################################################################################################################
        ...
        q_feature_rep, q_image_feature_rep = self.ED( q_feature, image_feature, None, None)#[b*4, len, 768]   #[b*4, num, 512]
        a_feature_rep, a_image_feature_rep = self.ED( a_feature, image_feature, None, None)
        qai_rep,aqi_rep = self.QAED(q_feature_rep,a_feature_rep,None,None)
...
####################################################################################################
        #
        qa_image_res = self.qa_AttFlat(qai_rep,None)   #[b,dim]
        aq_image_res = self.qa_AttFlat(aqi_rep,None)
        ...
        qa_proj_feat = aq_image_res + qa_image_res
...
class MCA_ED(nn.Module):
    def __init__(self,dim1,dim2):
        ...
        # self.dec_list = nn.ModuleList([SGA(256,256) for _ in range(6)])
