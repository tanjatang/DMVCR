...
################################################################################################################################
        ...
        path = '/home/tangxueq/MA_tang/r2c/models/saves/memory_cell1000.npz'
        self.memory_cell2 = torch.nn.Parameter(torch.Tensor(self.memory_cell_load(path, self.dict_size, 512)))
...
######################################################################################################
        ...
        #[96,4,50,1024]
        dim = 64    #3584##768*2 #sum([d for d, to_pool in [(reasoning_encoder.get_output_dim(), self.pool_reasoning),
...
##################################################################################################################
        ...
        qa_rep = self.GA_transformer(a_context_visual.view(a_context_visual.size(0)*a_context_visual.size(1),a_context_visual.size(2),a_context_visual.size(3)),q_context_visual.view(q_context_visual.size(0)*q_context_visual.size(1),q_context_visual.size(2),q_context_visual.size(3)),None,None) #[b x 4, q_len, 512]
        aq_rep = self.GA_transformer(q_context_visual.view(q_context_visual.size(0)*q_context_visual.size(1),q_context_visual.size(2),q_context_visual.size(3)),a_context_visual.view(a_context_visual.size(0)*a_context_visual.size(1),a_context_visual.size(2),a_context_visual.size(3)),None,None) #[b x 4, q_len, 512]

        qa_rep = torch.cat((qa_rep, aq_rep), dim=1)

        qa_rep_feature = self.qa_AttFlat(qa_rep, None)
        # aq_rep_feature = self.qa_AttFlat(aq_rep,None)
        # feature = qa_rep_feature + aq_rep_feature
        output_res = self.proj_norm(qa_rep_feature)
        ...
        # qa_rep = qa_rep[:,-1,:]
        # qa_res = self.Dictionary(qa_rep, self.memory_cell1)
...
###########################################################################################
        logits = self.final_mlp(output_res).squeeze(-1)#       [94,1024*2]
        # print(logits)
...
class MCA_ED(nn.Module):
    def __init__(self):
        ...
        self.dec_list = nn.ModuleList([SGA(512, 512) for _ in range(4)])
...
class GA_transformer(nn.Module):
    def __init__(self):
        ...
        self.enc_list = nn.ModuleList([SGA(768,768) for _ in range(3)])
