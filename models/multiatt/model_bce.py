...
############################################################################################################################3
        ...
        self.encoder_layer_vc = nn.TransformerEncoderLayer(d_model=128, nhead=8)
...
############################################################################################################################
        ...
        self.lstm_norm = torch.nn.BatchNorm1d(500)
        ...
        #[96,4,50,1024]
        dim = 128#768*2 #sum([d for d, to_pool in [(reasoning_encoder.get_output_dim(), self.pool_reasoning),
    ...
    def memory_cell_load(self,path):
        if os.path.isfile(path):
            ...
            memory_init = torch.rand(500, 128)/100
...
# #####################################################################################################3
        ...
        # print(output.size(),'ppppppppppppppppppppppppp')
...
#################################################################################################################
        # '''去掉4那个维度，用lstm'''
        # output_res = output_res.view(-1,4,128)
        # to_pool,(h,c) = self.lstm(output_res)
        # to_pool = to_pool[:,-1,:]
...
#########################################################
        ...
        logits = self.final_mlp(output_res).squeeze(-1)#.squeeze(2)#      [94,4,1024]

        class_probabilities = F.softmax(logits,dim = -1)
        # logits = self.l(logits).squeeze(-1)
        # print(logits, 'kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk')

        # class_probabilities = F.sigmoid(logits)#nn.Sigmoid(logits)
        # print(class_probabilities,'kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk')
