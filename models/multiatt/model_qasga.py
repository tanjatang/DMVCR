...
class AttentionQA(Model):
    def __init__(self,
        ...
        self.QA2SGA_transformer = QA2SGA_transformer()
    ...
    def forward(self,
        ...
        qaa_rep,qaq_rep = self.QA2SGA_transformer(
        ...
        qa_rep_feature = self.qa_AttFlat(qaa_rep, None)
        aq_rep_feature = self.aq_AttFlat(qaq_rep, None)
...
class QA2SGA(nn.Module):
    def __init__(self, dim1, dim2):
        super(QA2SGA, self).__init__()
        # self.hidden_size = dim1
    ...
    def forward(self, x, y, x_mask, y_mask,z):  #
        ''' x: image， y: context, '''

        # z = torch.cat((x, y), dim=1)
        z = self.norm1(z + self.dropout1(
            self.mhatt1(z, z, z, None)
        ))

        zy = self.norm2(z + self.dropout2(
            self.mhatt2(y, y, z, y_mask)
        ))

        zy = self.norm3(zy + self.dropout3(
            self.ffn(zy)
        ))

        zx = self.norm2(z + self.dropout2(
            self.mhatt2(x, x, z, x_mask)
        ))

        zx = self.norm3(zx + self.dropout3(
            self.ffn(zx)
        ))

        # z = torch.cat((zy,zx),dim=1)
...
# ---------------QAGA Transformer----------------------------------------
# ----------------------------------------------------------------------
class QA2SGA_transformer(nn.Module):
    def __init__(self):
        super(QA2SGA_transformer, self).__init__()
        self.enc_list = nn.ModuleList([QA2SGA(512, 512) for _ in range(3)])
        # self.position = PositionalEncoding(512,0.1,5000)
