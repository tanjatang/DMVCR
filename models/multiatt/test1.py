...
class AttentionQA(Model):
    def __init__(self,
        ...
        #
        # vgg16 = self.detector.backbone#.state_dict()
        ...
        self.detector.load_state_dict(torch.load('/home/tangxueq/tmp/test.th'))

        # for param in self.detector.state_dict():
        #     param[1].requires_grad = False
        # print(torch.load('/home/tangxueq/tmp/test.th'))
        # for k, v in torch.load('/home/tangxueq/tmp/test.th').items():
        #     print("keys:",k)
        # ipdb.set_trace()
        ...
        self.VLED = MCA_ED(768,512)
        self.QED = MCA_ED(768,768)
        self.AED = MCA_ED(768,768)
        ...
        #     if n not in self.detector.named_parameters():
        #     # print('jjjjjjjjjjjjjjjjjjjjjj',n,p)
        #         p.requires_grad = False
        #     else:
...
#------------visual grounding----------------------------------------------------------------------
        ...
        q_feature = question['bert'].view(question['bert'].size(0)*question['bert'].size(1),-1,question['bert'].size(-1)) #[b, 4xlen, 768]
        a_feature = answers['bert'].view(answers['bert'].size(0)*answers['bert'].size(1), -1, answers['bert'].size(-1)) #[b, 4xlen, 768]
        ...
        image_feature = image_feature.view(image_feature.size(0) * image_feature.size(1), image_feature.size(2),
                                           image_feature.size(3))

        q_rep, qi_rep = self.VLED(q_feature,image_feature,None,None)
        a_rep, ai_rep = self.VLED(a_feature,image_feature,None,None)
#-------------------commonsense encoder---------------------------------------------------------------------------
        qa_rep = torch.cat((q_rep,a_rep),dim=1)

        q_res, qqa_res = self.QED(q_rep,qa_rep,None,None)
        a_res, aqa_res = self.QED(a_rep,qa_rep,None,None)


#-----------------------------------------------------------Att reduce--------------------------------------------------------------------------

        qqa_rep_feature = self.qa_AttFlat(qqa_res, None)  # [batch x 4, 64]
        aqa_rep_feature = self.aq_AttFlat(aqa_res, None)  # [batch x 4, 64]
        qq_rep_feature = self.qa_AttFlat(q_res, None)  # [batch x 4, 64]
        aq_rep_feature = self.aq_AttFlat(a_res, None)  # [batch x 4, 64]
        qi_rep_feature = self.img_AttFlat(qi_rep, None)  # [batch x 4, 64]
        ai_rep_feature = self.img_AttFlat(ai_rep, None)  # [batch x 4, 64]

        fusion = qqa_rep_feature + aq_rep_feature + aqa_rep_feature + qq_rep_feature + qi_rep_feature + ai_rep_feature
...
class MCA_ED(nn.Module):
    def __init__(self,dim1,dim2):
        ...
        self.enc_list = nn.ModuleList([SA(dim1) for _ in range(3)])
        self.dec_list = nn.ModuleList([SGA(dim2, dim1) for _ in range(3)])
