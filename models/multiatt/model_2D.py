...
#################################################################################################################################
        ...
        #[96,4,50,1024]
        dim = 1024#768*2 #sum([d for d, to_pool in [(reasoning_encoder.get_output_dim(), self.pool_reasoning),
    ...
    def memory_cell_load(self,m1,m2):
        if os.path.isfile(m1) and os.path.isfile(m2):
            ...
            memory_init1 = torch.randn(self.dic_size, 512)/100  #randn
            memory_init2 = torch.randn(self.dic_size, 512) / 100
...
############ gru
        # Toutput, h_n = self.t_gru(Tqa_rep)
        # Toutput = Toutput[:,-1,:]
