...
class AttentionQA(Model):
    def __init__(self,
                 ...
                 hidden_dim_maxpool: int = 32, #1024,
...
################################################################################################################################
        ...
        self.memory_cell1 = torch.nn.Parameter(torch.Tensor(self.memory_cell_load(path, self.dict_size, 256)))
        print(self.memory_cell1)

        path = '/home/tangxueq/MA_tang/r2c/models/saves/memory_cell1000.npz'
        self.memory_cell2 = torch.nn.Parameter(torch.Tensor(self.memory_cell_load(path, self.dict_size, 256)))
        print(self.memory_cell2)

        self.memory_cell3 = torch.nn.Parameter(torch.Tensor(self.memory_cell_load(path, self.dict_size, 256)))
        print(self.memory_cell3)
...
######################################################################################################
        ...
        #[96,4,50,1024]
        dim = 768#1024 #* 4     #3584##768*2 #sum([d for d, to_pool in [(reasoning_encoder.get_output_dim(), self.pool_reasoning),
...
#############################################第三个字典
        qa_text = torch.cat((question['bert'],answers['bert']),dim = 2)
        len = qa_text.size(2)
        qa_text = qa_text.view(-1,len,768)

        qa_text_rep, (h, z) = self.cvlstm(qa_text)
        qa_text_rep = qa_text_rep[:,-1,:]


        qa_text_res = self.Dictionary(qa_text_rep,self.memory_cell3)
        # print(qa_text_res.size(),'hhhhhhhhhhhhhhhhhhhhhhhh')
        qa_text_res = qa_text_res.view(-1,4,256)


        #############################global average pooling
        # vc_qa_rep = self.avg_pool(qa_visual_context).squeeze(1)
        # cv_qa_rep = self.avg_pool(qa_context_visual_inp).squeeze(1)
        #
        #
        ...
        output_res = torch.cat((vc_qa_res, cv_qa_res,qa_text_res), dim=-1)  # [96, 4, 1024*2]

        # print(output_res.size(), qa_text_res.size(), 'kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk')
