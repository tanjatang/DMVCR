...
##################################################################################################################
        ...
        qaa_feature_rep, aqa_feature_rep = self.ED( qa_feature, a_rep.view(a_rep.size(0)*4,-1,a_rep.size(-1)), None, None)
        qai_feature_rep, iqa_feature_rep = self.ED( qa_feature, image_feature, None, None)
...
####################################################################################################
        ...
        qaa_image_res = self.image_AttFlat(iqa_feature_rep,None)   #[b,dim]
        qai_res = self.qa_AttFlat(qai_feature_rep,None)
        qa_res = self.qa_AttFlat(qaa_feature_rep,None)
        aqa_res = self.qa_AttFlat(aqa_feature_rep,None)


        # qa_feature_res = qa_feature_res.view(-1,4,64)
        # qa_image_res = qa_image_res.view(-1,4,64)

        qa_proj_feat = qaa_image_res + qa_res + qai_res +aqa_res
        qa_proj_feat = qa_proj_feat.view(-1,4,64)
