...
class AttentionQA(Model):
    def __init__(self,
        ...
        # self.detector.load_state_dict(torch.load('/home/tangxueq/tmp/baseline/best.th'))
        ...
        self.aq_AttFlat = AttFlat(768)
        # self.GA_transformer_qa = GA_transformer(768,768)
        # self.GA_transformer_vqa = GA_transformer(768,512)
        self.QAGA_transformer_qa = QAGA_transformer(768, 768)
        self.QAGA_transformer_aq = QAGA_transformer(768, 768)
        self.GA_transformer_vqa = GA_transformer(768, 512)
        # self.CA = CA_transformer()
        self.AvgPool = nn.AdaptiveAvgPool1d(1)
        self.encoder_layer_vf = nn.TransformerEncoderLayer(d_model=512, nhead=8)
        self.transformer_encoder_vf = nn.TransformerEncoder(self.encoder_layer_vf, num_layers=3)
...
#------------get refine visual feature and refine qa language feature-----------------------------------------------------------------------

        refine_visual_feature = self.transformer_encoder_vf(obj_reps['obj_reps'])# [batch, num, 512]

        guaid_qa = self.QAGA_transformer_qa(answers['bert'].view(answers['bert'].size(0)*answers['bert'].size(1),answers['bert'].size(2),answers['bert'].size(3)),
                                          question['bert'].view(question['bert'].size(0)*question['bert'].size(1),question['bert'].size(2),question['bert'].size(3)),None,None) #[b x 4, q_len, 768]

        guaid_aq = self.QAGA_transformer_aq(
            question['bert'].view(question['bert'].size(0) * question['bert'].size(1), question['bert'].size(2),
                                 question['bert'].size(3)),
            answers['bert'].view(answers['bert'].size(0) * answers['bert'].size(1), answers['bert'].size(2),
                                  answers['bert'].size(3)), None, None)  # [b x 4, q_len, 768]

        refine_visual_feature = torch.unsqueeze(refine_visual_feature,1)
        refine_visual_feature = refine_visual_feature.repeat(1,4,1,1)

        guaid_vqa = self.GA_transformer_vqa(refine_visual_feature.view(refine_visual_feature.size(0)*refine_visual_feature.size(1),refine_visual_feature.size(2),refine_visual_feature.size(3)), guaid_qa,None,None)    #[b x 4, q_len, 768]
        guaid_vaq = self.GA_transformer_vqa(refine_visual_feature.view(refine_visual_feature.size(0)*refine_visual_feature.size(1),refine_visual_feature.size(2),refine_visual_feature.size(3)), guaid_aq,None,None)    #[b x 4, q_len, 768]


        # print(guaid_aq.shape,)
#-----------------------------------------------------------Att reduce--------------------------------------------------------------------------
        q_len = question_mask.size(-1)
        a_len = answer_mask.size(-1)
        # qa_rep_feature = self.qa_AttFlat(guaid_vqa, question_mask[..., None].view(-1, q_len, 1)) # [batch x 4, 64]
        # aq_rep_feature = self.aq_AttFlat(guaid_vaq, answer_mask[..., None].view(-1, a_len, 1)) # [batch x 4, 64]
        qa_rep_feature = self.qa_AttFlat(guaid_vqa, None)  # [batch x 4, 64]
        aq_rep_feature = self.aq_AttFlat(guaid_vaq, None)  # [batch x 4, 64]

        fusion = qa_rep_feature+aq_rep_feature

        output_res = self.proj_norm(fusion)
...
class GA_transformer(nn.Module):
    def __init__(self,dim1,dim2):
        ...
        self.enc_list = nn.ModuleList([SGA(dim1, dim2) for _ in range(3)])
...
class QAGA_transformer(nn.Module):
    def __init__(self,dim1,dim2):
        ...
        self.enc_list = nn.ModuleList([QASGA(dim1,dim2) for _ in range(3)])
        # self.position = PositionalEncoding(512,0.1,5000)
